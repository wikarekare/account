...
class Preprocess
  ...
  def go(tmp_stats_file, gplot_filename)
    ...
    @tmp_stats_file, @gplot_filename = tmp_stats_file, gplot_filename
    ...
    my = Mysql::new(@mysql_conf.host, @mysql_conf.dbuser, @mysql_conf.key, @mysql_conf.db)
      res = my.query("select free_rate from plan where plan_id = -1")
      res.each do |row|
        #Free traffic rate, measured in bytes every 10s
        @interval_byte_count=(INTERVAL_BYTE_COUNT_100 * row[0].to_f).round
      end
      
      #Get the actual values
      res = my.query("select hostname, sum(bytes_in + bytes_out)/#{GBYTE} " +
                     "from log_summary " +
                     "where " +
      ...
      res.each do |row|
        row[0].capitalize! if row[0] =~ /^link/
        sum_giga_bytes[row[0]] = [ row[1].to_f, 0.0 ] #Set the actual GB usage
      end
      res.free

      #Get the values, above the threshold in each interval.
      res = my.query("select hostname, sum(bytes_in + bytes_out - #{@interval_byte_count})/#{GBYTE} " +
                     "from log_summary " +
                     "where " +
                       "log_timestamp >= '#{@start_when.strftime("%Y-%m-%d %H:%M:%S")}' and " + 
       ...
       res.each do |row|
         row[0].capitalize! if row[0] =~ /^link/
         sum_giga_bytes[row[0]] ||= [ 0.0, 0.0 ]
         sum_giga_bytes[row[0]][1] = row[1].to_f #Set the GB usage above the interval threshold
       end
      res.free
    my.close
    ...
    sorted.each do |hostname, usage|  
      unless hostname =~ /^10\..*\.0$/ || hostname =~ /^Link/
        @key_by_index << hostname
