...
def fetch_site_plans
  ...
  if (my = Mysql::new(@mysql_conf.host, @mysql_conf.dbuser, @mysql_conf.key, @mysql_conf.db)) != nil
    res = my.query("select site_name, plan_id, base_gb, extended_gb, base_price, extended_unit_price, excess_unit_price from customer, plan where customer.plan = plan.plan_id")
    if res != nil
      res.each do |row|
        # site_plan[site_name] =
        site_plan[row[0]] = {:plan_id => row[1].to_i,    :base_gb => row[2].to_i,             :extended_gb => row[3].to_i, 
                             :base_price => row[4].to_f, :extended_unit_price => row[5].to_f, :excess_unit_price => row[6].to_f}
      end
    end
    my.close
  end
...
if ARGV[1] != nil
  ...
  output_name = input_name.gsub(/usage_/,'bill_')
end
