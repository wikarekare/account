      ...
      if line[0,1] != '#'
        ...
        if tokens[7] == target && !ignore?(tokens[8]) && (t = Time.parse(tokens[0])) >= @start && t <= @end
          ...
          if(ports[tokens[10]] == nil) #no existing entry for this port
            ...
            ports[tokens[10]] = (ports[tokens[10]] + tokens[13].to_i + tokens[14].to_i )
          end
        end
      end
    end
...
EOF
  ...
  def port_color(port)
    case port.to_i
    when 80,443; 'green' #Wed
    when 25,993,143,220,585,109,110,473,995; 'yellow' #Mail
    when 1194,1723, 500,1194; 'gold'
    when 22,23,2022; 'pink' #ssh
    when 6881..6999; 'red' #BT
    when 53,123; 'aquamarine' #Ntp, DNS
    when 21,20; 'magenta' #ftp
    else 'grey'
    end
  end
end
