...
EOF
  ...
  private def collect_data
    ...
    (@start_time.to_i..@end_time.to_i).step(86400) do |d|
      ...
      cmd = "#{FLOW_CAT} #{filename} | #{FLOW_NFILTER} -F network -v NETWORK=#{@ip_net.to_s} | #{FLOW_PRINT} -f 25"
      puts cmd if @debug
...
=end
  ...
  private def parse_flow_print_25_line(line)
    ...
    if @end_time > fields_start_time && @start_time < fields_end_time
      ...
      local_ip, direction, remote_ip = local_address?(fields[3],fields[6])
      if @host_traffic[local_ip] == nil
        @host_traffic[local_ip] = direction ? [fields[12].to_i, 0] : [0, fields[12].to_i]
      elsif ! ignore?(remote_ip)
        if direction #Pick local hosts
          @host_traffic[local_ip][0] += fields[12].to_i
        else
          @host_traffic[local_ip][1] += fields[12].to_i
        end
      end
    end
  end
  ...
  private def prepare_data
    ...
    sorted_host_traffic = @host_traffic.sort { |a, b| a[0] <=> b[0] }
    count = 0
