        ...
        File.open( filename ).each do |line|
          if line[0,1] != '#'
            ...
            if tokens[7] == target && !ignore?(tokens[8]) && (t = Time.parse(tokens[0])) >= @start && t <= @end
              ...
              if(ports[port] == nil) #no existing entry for this port
                ports[port] =  [  tokens[14].to_i, tokens[13].to_i, tokens[6].to_i ] #As array of in, out
              else
                ports[port][1] += tokens[13].to_i #in
                ports[port][0] += tokens[14].to_i #out
              end
            end
          end
        end
...
EOF
  ...
  private def port_name(port, protocol)
    ...
    when 6,17
      ...
      when 22; 'ssh'
      when 23; 'telnet' 
      when 6881..6999; "BT-#{port}" #BT
      when 53; 'DNS'
      when 123; 'ntp' #Ntp, DNS
      when 115; 'sftp'
      when 21; 'ftp'
      when 20; 'ftp-data' #ftp
      when 161; 'snmp'
      when 162; 'snmptrap'
      when 514; 'syslog'
      when 0...1024; "#{port}"
